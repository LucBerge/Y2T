...
import Y2T_Download, Y2T_Presentation, Y2T_Nfo, Y2T_Torrent
import Y2T_Log as log

nfo_package = "mediainfo"
torrent_package = "ctorrent"
...
class Upload:
	...
	def upload(self, album, cover, annee=None, mois=None, duree_max=600):
		...
		if(nfo_package in os.popen("dpkg -l | grep " + nfo_package).read()):
			...
			log.erreur("Impossible de créer le fichier \"" + album + ".nfo\". Vous devez installer mediainfo")

		#Création du .torrent
		if(torrent_package in os.popen("dpkg -l | grep " + torrent_package).read()):
			self.torrent.create(album)
		else:
			log.erreur("Impossible de créer le fichier \"" + album + ".torrent\". Vous devez installer ctorrent")
